...
@Component({
    selector: 'app-paymentDetailNextCredit',
    templateUrl: './paymentDetailNextCredit.component.html',
    styleUrls: ['./paymentDetailNextCredit.component.scss']
})
export class PaymentDetailNextCreditComponent extends MasterProvider implements OnInit {
...
//            this.getUser();
            ...
            if (payment_status == "Failed"){
                ...
                this.doHttpPost("/api/paymentDetail/remainingPayment", orderArray, true).subscribe((data: any) => {
                    ...
                    console.log(data);
                    let custCreditObj = {
                        custId: this.getSession().id_mongo,
                        ordAmt: this.appService.Data.totalAmount,
                        operation: "add"
                    }
                    let jsonArr = [];
                    jsonArr.push(custCreditObj);
                    console.log(jsonArr);
                    this.doHttpPost("/api/paymentDetail/updateCreditAmt", jsonArr, true).subscribe((data: any) => {
                        data = data.response;
                        console.log("credittttttttttttttttttttttttttttttttttttt");
                        console.log(data);
