...
export class CustomisedQuotationListDataSource extends MasterProvider implements DataSource<any> {
    
    API_LAZY_LST = "/api/quotationEnquiry/listOfExpQuot";
    API_LAZY_LST_COUNT = "/api/quotationEnquiry/getCountForExpQuot";
    ...
    loadLazyLst(lazyReq: LazyLoadRequest) {
        ...
        this.getList(lazyReq).pipe(
            ...
            .subscribe((oprs: any[]) => this.decrypt(oprs));
    }

    decrypt(oprs:any){
        oprs = oprs.response;
        this.oprsSubject.next(oprs)
    }
